pipeline {
  ...
  environment {
        ...
        SONARQUBE_TAGS = "www.eea.europa.eu,demo-www.eea.europa.eu,www.eea.europa.eu-en,insitu.copernicus.eu,clmsdemo.devel6cph.eea.europa.eu,forest.eea.europa.eu,land.copernicus.eu,demo-ied.eea.europa.eu,www.ied.eea.europa.eu-en,climate-advisory-board.europa.eu,demo-water.devel5cph.eea.europa.eu-freshwater,water.europa.eu-freshwater,biodiversity.europa.eu"
    }
        ...
        node(label: 'swarm') {
          script {
            ...
            withSonarQubeEnv('Sonarqube') {
              sh "export PATH=$PATH:${scannerHome}/bin:${nodeJS}/bin; sonar-scanner -Dsonar.python.xunit.skipDetails=true -Dsonar.python.xunit.reportPath=coverage/junit-results/testreports/*.xml -Dsonar.python.coverage.reportPaths=coverage/coverage.xml -Dsonar.sources=./pas -Dsonar.exclusions=**/tests/**,**/setup.py -Dsonar.projectKey=$GIT_NAME-$BRANCH_NAME -Dsonar.projectVersion=$BRANCH_NAME-$BUILD_NUMBER"
