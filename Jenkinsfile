            ...
            node(label: 'docker') {
              script {
                try {
                  ...
                  sh '''docker run -i --name="$BUILD_TAG-python3" -e GIT_BRANCH="$BRANCH_NAME" -e ADDONS="$GIT_NAME[test]" -e DEVELOP="src/$GIT_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" -e PLONE_VERSIONS="requests=>=2.30.0,<3.0" eeacms/plone-test:5-python3 -v -vv -s $GIT_NAME'''
        ...
        node(label: 'swarm') {
          script{
            ...
            withSonarQubeEnv('Sonarqube') {
                sh "export PATH=$PATH:${scannerHome}/bin:${nodeJS}/bin; sonar-scanner -Dsonar.python.xunit.skipDetails=true -Dsonar.sources=./src -Dsonar.projectKey=$GIT_NAME-$BRANCH_NAME -Dsonar.projectVersion=$BRANCH_NAME-$BUILD_NUMBER"
